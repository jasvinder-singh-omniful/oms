...
func (h *UserHandler) CreateUserHandler(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&body); err != nil {
		log.ErrorfWithContext(ctx, logTag+" Failed to bind JSON", err)
	...
	if err != nil {
		c.JSON(http.StatusBadRequest.Code(), response.ErrorResponse("error when hashing password ", err))
	}
...
func (h *UserHandler) GetUserByEmailHandler(c *gin.Context){
	...
	if err := c.ShouldBindJSON(&body); err != nil {
		c.JSON(http.StatusBadRequest.Code(), response.ErrorResponse("please enter email correctly ", err))
		return
	}
	...
	if err != nil {
		c.JSON(500, response.ErrorResponse("error when getting user by email ", err))
		return
	}
...
func (h *UserHandler) GetUserByIdHandler(c *gin.Context){
	...
	if err != nil {
		c.JSON(500, response.ErrorResponse("error when getting user by id ", err))
		return
	}
